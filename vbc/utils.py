...
def uvbc():
    ...
    desired_interval = 1.0 / frames_per_second
    ...
    while True:
        ...
        # Yield the frame in the format required by StreamingHttpResponse
        yield (
            b"--frame\r\n"
        ...
        # Calculate sleep time to achieve the desired frame rate
        sleep_time = max(0, desired_interval - elapsed_time)

        # Adjust sleep time if needed to achieve the desired frames per second
        sleep(sleep_time)

        key = cv2.waitKey(1)

    # Release the camera and COM
